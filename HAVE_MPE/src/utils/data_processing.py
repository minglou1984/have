...
def _get_obs_component_dim(args):
    own_feats_dim, ally_feats_dim, enemy_feats_dim = args.obs_component  # [4, (4, 2), (5, 2)]
    enemy_feats_dim = np.prod(enemy_feats_dim)
    ally_feats_dim = np.prod(ally_feats_dim)
    return own_feats_dim, ally_feats_dim, enemy_feats_dim
...
def do_data_augmentation(args, batch: EpisodeBatch, augment_times=2):
    ...
    for t in range(augment_times):
        ...
        own_feats, ally_feats, enemy_feats = th.split(obs, obs_component_dim, dim=-1)
        ...
        permuted_obs = th.cat([own_feats, permuted_ally_feat, permuted_enemy_feat], dim=-1)
