...
class Scenario(BaseScenario):
    def make_world(
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            agent.silent = True
            world.agents_infos[i, 4] = 0.08 # size
        ...
        world.landmarks = world.preys # --------------for TransfQmix--------------
        for i, prey in enumerate(world.preys):
            prey.name = 'prey %d' % i
            prey.collide = False
            prey.movable = True
            prey.silent = True
            world.preys_infos[i, 4] = 0.04 # size
    ...
    def benchmark_data(self, agent, world):
        ...
        info['catch_preys_rate'] = catched_preys_n / world.num_preys
        return info
    ...
    def generate_observation(self, world):
        ...
        prey_label = np.tile(np.array([[[0., 1.]]]), (world.num_agents, world.num_preys, 1))  # (n,m,1)

        obs_agents = np.concatenate([obs_agents, agent_label], axis=-1)
        obs_preys = np.concatenate([obs_preys, prey_label], axis=-1)

        # flatten
        obs_agents_flatten = obs_agents.reshape(world.num_agents, -1)
        obs_preys_flatten = obs_preys.reshape(world.num_agents, -1)

        obs_n = np.concatenate((self_info, obs_agents_flatten, obs_preys_flatten), axis=-1)
        world.obs_n = obs_n
        return obs_n
