...
# Fully Observation: Observe all landmarks and agents with label (1,1) of self,
# (1,0) of other agents, (0,0) of other landmarks in environments
# Shared reward
class Scenario(BaseScenario):
    def make_world(
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.collide = True
            agent.silent = True
            world.agents_infos[i, 4] = 0.04 # size, 0.04 for 5CN, 0.03 for CN 40
            agent.size = world.agents_infos[i, 4]
            world.agents_infos[i, 5] = 99 # observe distance
            # agent.accel = 5.0
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            world.landmarks_infos[i, 4] = 0.08 # size, 0.08 for 5CN, 0.03 for CN 40
    ...
    def reset_world(self, world):
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            #     for j in range(i):
            #         while True:
            #             if np.sqrt(np.sum(np.square(landmark.state.p_pos - world.landmarks[j].state.p_pos))) > landmark.size*3:
            #                 break
            #             else:
            #                 world.landmarks_infos[i, :2] = world.np_random.uniform(-world.range_p, +world.range_p,
            #                                                                        world.dim_p)
    ...
    def generate_observation(self, world):
        """
        Ex: agent 1 for agent 1:    (vel_x, vel_y, pos_x, pos_y)
        Ex: agent 2 for agent 1:    (dx(a1,a2), dy(a1,a2), 1)
        Ex: landmark 1 for agent 1: (dx(a1,l1), dy(a1,l1), 0)
        ...
        # self_info = np.concatenate([agents_vel, agents_pos, self_label], axis=-1)
        ...
        obs_agents = obs_agents[np.eye(world.num_agents) == 0].reshape(world.num_agents, -1,
                                                                       2)  # (n_agents, n_agents-1, 2)
        # label (1,0) with agents and (0,0) with landmarks
        agent_label = np.tile(np.array([[[1., 0.]]]), (world.num_agents, world.num_agents - 1, 1))  # (n,n-1,1)
        landmark_label = np.tile(np.array([[[0., 1.]]]), (world.num_agents, world.num_landmarks, 1))  # (n,m,1)
        ...
        obs_n = np.concatenate((self_info, obs_agents_flatten, obs_landmarks_flatten), axis=-1)
        world.obs_n = obs_n
        return obs_n
