...
from modules.mixers.qtransformer_v0 import TransformerMixer
...
class NQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        elif args.mixer == "transformer":
            self.mixer = TransformerMixer(args)
        elif args.mixer == 'dynamic_qmix':
            self.mixer = DynamicMixer(args)
        else:
            raise "mixer error"
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        # loss = (0.5 * masked_td_error.pow(2)).sum() / mask.sum()
        ...
        if self.args.target_update_interval > 1 and (episode_num - self.last_target_update_episode) / self.args.target_update_interval >= 1.0:
    ...
    def _update_targets_hard(self):
        ...
        self.logger.console_logger.info("Updated target network (Hard)")
