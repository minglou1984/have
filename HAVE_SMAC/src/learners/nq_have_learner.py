...
from modules.mixers.set_mix import SetMixer
...
class NQHAVELearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        if args.mixer == "set_qmix":
            self.mixer = SetMixer(args)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int):
        ...
        is_alive = (1 - avail_actions[..., 0].type(th.float)) # (bs, seq_len, n_agents)
            ...
            if self.args.mixer.find("qmix") != -1 and self.enable_parallel_computing:
                targets = self.pool.apply_async(
                    ...
                    (self.target_mixer, target_max_qvals, batch, rewards, terminated, mask, self.args.gamma,
                     self.args.td_lambda, is_alive, True, self.args.thread_num, False, None)
                )
            else:
                targets = calculate_n_step_td_target(
                    self.target_mixer, target_max_qvals, batch, rewards, terminated, mask, self.args.gamma,
                    self.args.td_lambda, is_alive
                )

        # Set mixing net to training mode
        self.mixer.train()
        # Mixer
        chosen_action_qvals = self.mixer(chosen_action_qvals, batch["state"][:, :-1], batch["obs"][:, :-1], batch["actions"][:, :-2], is_alive[:, :-1])
...
def calculate_n_step_td_target(target_mixer, target_max_qvals, batch, rewards, terminated, mask, gamma, td_lambda, is_alive,
    ...
    with th.no_grad():
        ...
        target_max_qvals = target_mixer(target_max_qvals, batch["state"], batch["obs"], batch["actions"][:, :-1], is_alive)

        if q_lambda:
            raise NotImplementedError
            qvals = th.gather(target_mac_out, 3, batch["actions"]).squeeze(3)
            qvals = target_mixer(qvals, batch["obs"], is_alive)
