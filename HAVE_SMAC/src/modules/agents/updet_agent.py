...
class SelfAttention_for_large(nn.Module):
    def __init__(self, emb, heads=8, mask=False):
        super(SelfAttention_for_large, self).__init__()
    ...
    def forward(self, x, mask):
        ...
        # keys = keys.permute(0, 2, 1, 3).clone().reshape(b * h, t, e)
        # queries = queries.permute(0, 2, 1, 3).clone().reshape(b * h, t, e)
        # values = values.permute(0, 2, 1, 3).clone().reshape(b * h, t, e)
        ...
        # dot = torch.bmm(queries, keys.permute(0, 2, 1).clone())
        ...
        out = torch.bmm(dot, values).reshape(b, h, t, e)

        # swap h, t back, unify heads
        # out = out.permute(0, 2, 1, 3).clone().reshape(b, t, h * e)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, emb, heads, mask, ff_hidden_mult=4, dropout=0.0):
        ...
        self.attention = SelfAttention_for_large(emb, heads=heads, mask=mask)
        self.mask = mask
