...
class BasicMAC:
    ...
    def save_models(self, path):
        ...
        if self.args.name == 'saft_qmix':
            th.save(self.agent.darnet.state_dict(), "{}/agent_darnet.th".format(path))

    def load_models(self, path):
        self.agent.load_state_dict(th.load("{}/agent.th".format(path), map_location=lambda storage, loc: storage))
        if self.args.name == 'saft_qmix':
            self.agent.darnet.load_state_dict(th.load("{}/agent_darnet.th".format(path), map_location=lambda storage, loc: storage))
